...
bool init(){if(g_overlay)return true;g_game=FindWindowW(nullptr,L"Euro Truck Simulator 2");if(!g_game)return false;WNDCLASSW wc{};wc.lpfnWndProc=wnd_proc;wc.hInstance=GetModuleHandleW(nullptr);wc.lpszClassName=kClassName;wc.hCursor=LoadCursor(nullptr,IDC_ARROW);RegisterClassW(&wc);g_overlay=CreateWindowExW(WS_EX_TOOLWINDOW|WS_EX_NOACTIVATE,kClassName,L"ETS2 Examiner",WS_POPUP,0,0,kWidth,kHeight,nullptr,nullptr,GetModuleHandleW(nullptr),nullptr);if(!g_overlay)return false;position_overlay();ShowWindow(g_overlay,SW_HIDE);refresh_players();return true;}
...
void tick(){if(!g_overlay){init();return;}if(g_game&&!IsWindow(g_game))g_game=FindWindowW(nullptr,L"Euro Truck Simulator 2");if(GetAsyncKeyState(VK_F8)&0x8000){ULONGLONG n=GetTickCount64();if(n-g_last_toggle>300){g_last_toggle=n;toggle();}}if(g_visible){refresh_players();position_overlay();ShowWindow(g_overlay,SW_SHOWNOACTIVATE);}}
